...
function promptFor(file) {
  return [
    ...
    '',
    '  D) Encyclopedia of Life / EOL (sourceType "research-site"):',
    '     curl -sL "https://eol.org/api/search/1.0.json?q=<SCINAME>" -o $WORK/eol_s.json  (take results[0].id = pageID)',
    '     curl -sL "https://eol.org/api/pages/1.0/<pageID>.json?details=true&images_per_page=20" -o $WORK/eol_p.json',
    '     For each taxonConcept.dataObjects[] whose dataType contains "StillImage": use dataObject.mediaURL',
    '     (image), dataObject.license (usually a creativecommons.org URL), and rightsHolder/agents (attribution).',
    '',
    '  E) Flickr (sourceType "other") — ONLY if a Flickr API key is available:',
